...
document.addEvent('domready', function(){
	...
	setInterval(cloth.breeze.bind(cloth), 6666);
});
...
// 		spacing = (max_dim - (Math.max(x_offset, y_offset) * 2)) / max_points;
 		spacing = canvas.width/4 /max_points;
...
Cloth.prototype = {
	...
	update: function() {
		...
		//randomly modify points by wind
// 		this.points[(Math.random()*num_y).floor()][(Math.random()*num_x).floor()].breeze();
